...
func (r *CategoryRepoPG) List(ctx context.Context, userID *string) ([]*domain.Category, error) {
	var query string
	var args []interface{}
	if userID == nil {
		query = `SELECT id, name, user_id FROM categories WHERE user_id IS NULL ORDER BY name`
		args = nil
	} else {
		query = `SELECT id, name, user_id FROM categories WHERE user_id IS NULL OR user_id = $1 ORDER BY name`
